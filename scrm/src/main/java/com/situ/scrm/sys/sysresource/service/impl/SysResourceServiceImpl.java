...
public class SysResourceServiceImpl implements SysResourceService {
	...
	private Map<String, List<SysResource>> buildResourceMap() {
		...
		Map<String, List<SysResource>> resourceMap = new HashMap<String, List<SysResource>>(); //查询出来的数据放到Map中
		if (resourceList != null) {
			for (SysResource sysResource : resourceList) {//遍历取出来的数据
				String parentCode = sysResource.getParentCode();  //得到parentCode作为Map的主键
				List<SysResource> list = resourceMap.get(parentCode);  //通过parentCode取出系统资源数据
				if (list == null) {
					list = new ArrayList<SysResource>();
				}
				list.add(sysResource);   //遍历出的所有数据放到list里面
				resourceMap.put(parentCode, list);  //用parentCode作为key   list作为value   放进Map里面
			}
		}
		return resourceMap;
	}
